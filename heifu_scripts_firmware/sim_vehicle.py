...
#>> Adapt SYSID_THISMAV parameter to the correspondent instance
param_file = os.path.join(find_autotest_dir(), 'default_params', cmd_opts.frame + '.parm') 

with open(param_file, "r") as f:
    lines = f.readlines()
with open(param_file, "w") as f:
    for line in lines:
        if "SYSID_THISMAV" not in line.strip("\n"):
            f.write(line)
    f.write("SYSID_THISMAV\t" + str(cmd_opts.instance+1))
#<< Adapt SYSID_THISMAV parameter to the correspondent instance
